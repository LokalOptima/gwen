...
run: $(BUILD_DIR)/gwen $(MODEL)
	$(BUILD_DIR)/gwen --model $(MODEL) --prompt "$(PROMPT)" --n-predict $(N) --greedy

run-mtp: $(BUILD_DIR)/gwen $(MODEL) $(MTP) $(MTP_HEAD)
	$(BUILD_DIR)/gwen --model $(MODEL) --mtp $(MTP) --mtp-lm-head $(MTP_HEAD) \
		--prompt "$(PROMPT)" --n-predict $(N)

# ── Benchmark ────────────────────────────────────────────────────────
bench: $(BUILD_DIR)/gwen $(MODEL)
	$(BUILD_DIR)/gwen --model $(MODEL) --prompt "$(PROMPT)" --n-predict $(N) --greedy --benchmark

bench-mtp: $(BUILD_DIR)/gwen $(MODEL) $(MTP) $(MTP_HEAD)
	$(BUILD_DIR)/gwen --model $(MODEL) --mtp $(MTP) --mtp-lm-head $(MTP_HEAD) \
		--prompt "$(PROMPT)" --n-predict $(N) --benchmark
