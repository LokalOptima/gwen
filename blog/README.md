...
35. [Closing the Gap: dp4a GEMV, Q8_0, and Native IQ4_XS](35-closing-the-gap-dp4a-iq4xs.md) — Wiring existing dp4a kernels (+117%), writing Q8_0 dp4a for ssm_out, native IQ4_XS with `__byte_perm` lookup. 45.7 → 118 tok/s (2.58×), matching llama.cpp. ncu profiling shows 75-82% bandwidth efficiency across all GEMV types.
36. [Beating llama.cpp on Both Fronts](36-beating-llama-cpp-on-both-fronts.md) — gwen-bench tool, three dead-end kernel fusion attempts, IQ4_XS prefill bottleneck (42.6% of time), MMQ GEMM one-liner fix. Prefill 2,736 → 5,997 tok/s (2.19×), decode 127 tok/s. Beats llama.cpp on both pp (+4.5%) and tg (+7.7%).
37. [Vendoring llama.cpp and Dead Code Cleanup](37-vendoring-llama-cpp.md) — Copied 17 llama.cpp headers into GWEN tree, eliminated external dependency. Removed 1,145 lines of dead code (failed fusion experiments, broken native flash attn). Added correctness test suite.
