...
// ============================================================
// Fused F32-input RMSNorm + Q8_1 Quantize (+ optional FP16)
// ============================================================
// 32 threads (single warp) — no __syncthreads__ needed.
// Phase 1: sum of squares with warp shuffle reduction
// Phase 2: iterate Q8_1 blocks, apply norm+weight, quantize, write
// Eliminates intermediate FP16 buffer and one kernel launch vs separate approach.
__global__ void __launch_bounds__(32)
kernel_rmsnorm_f32_input_quantize_q8_1(
    ...
    // Phase 2: process each Q8_1 block (32 elements, 1 per thread)
    int n_blocks = dim / 32;
    for (int blk = 0; blk < n_blocks; blk++) {
        int elem_idx = blk * 32 + lane;
        float val = x_f32[elem_idx] * rms_inv * weight[elem_idx];

        if (y_fp16) y_fp16[elem_idx] = __float2half(val);

        // Q8_1: find block amax
        float amax = fabsf(val);
        #pragma unroll
        for (int offset = 16; offset > 0; offset >>= 1)
            amax = fmaxf(amax, __shfl_xor_sync(0xFFFFFFFF, amax, offset));
        ...
        y_q8[blk].qs[lane] = q;
        if (lane == 0)
            y_q8[blk].ds = __halves2half2(__float2half(d), __float2half(sum));
    }
}

// Batch2: F32-input fused RMSNorm + Q8_1 for two tokens (2 blocks × 32 threads)
__global__ void __launch_bounds__(32)
kernel_rmsnorm_f32_input_quantize_q8_1_batch2(
        const float* __restrict__ x0_f32, const float* __restrict__ x1_f32,
        const float* __restrict__ weight,
        block_q8_1* __restrict__ y_q8_0, block_q8_1* __restrict__ y_q8_1,
        half* __restrict__ y_fp16_0, half* __restrict__ y_fp16_1,
        int dim, float eps) {
    const float* x_f32 = (blockIdx.x == 0) ? x0_f32 : x1_f32;
    block_q8_1* y_q8 = (blockIdx.x == 0) ? y_q8_0 : y_q8_1;
    half* y_fp16 = (blockIdx.x == 0) ? y_fp16_0 : y_fp16_1;
...
void gwen_rmsnorm_f32_input_quantize_q8_1(const float* x_f32, const float* weight,
                                            ...
                                            int dim, float eps, cudaStream_t stream) {
    kernel_rmsnorm_f32_input_quantize_q8_1<<<1, 32, 0, stream>>>(
        x_f32, weight, static_cast<block_q8_1*>(y_q8), y_fp16, dim, eps);
    GWEN_CHECK_CUDA(cudaGetLastError());
}
...
void gwen_rmsnorm_f32_input_quantize_q8_1_batch2(
        ...
        int dim, float eps, cudaStream_t stream) {
    kernel_rmsnorm_f32_input_quantize_q8_1_batch2<<<2, 32, 0, stream>>>(
        x0_f32, x1_f32, weight,
        static_cast<block_q8_1*>(y_q8_0), static_cast<block_q8_1*>(y_q8_1),
        y_fp16_0, y_fp16_1, dim, eps);
    GWEN_CHECK_CUDA(cudaGetLastError());
}
